...
/**
 * @notice Fixed deployment script with proper storage version initialization
 */
contract SimpleDeployFixed is Script {
    function run() external {
        // Use anvil private key directly
        uint256 deployerPrivateKey = 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;
        address deployer = vm.addr(deployerPrivateKey);
        
        console.log("=== Fixed Deployment with Storage Version ===");
        ...
        // **CRITICAL FIX**: Initialize storage version BEFORE any facet initialization
        console.log("Initializing storage version...");
        bytes32 storageSlot = keccak256("ScrumPokerStorage.DiamondStorage");
        bytes32 versionSlot = bytes32(uint256(storageSlot) + 0); // version é o primeiro campo
        
        // Check current version
        uint256 currentVersion = uint256(vm.load(address(diamond), versionSlot));
        console.log("Current storage version:", currentVersion);
        
        if (currentVersion == 0) {
            console.log("Setting storage version to:", ScrumPokerStorage.CURRENT_STORAGE_VERSION);
            bytes32 newVersionValue = bytes32(ScrumPokerStorage.CURRENT_STORAGE_VERSION);
            vm.store(address(diamond), versionSlot, newVersionValue);
            
            // Verify the version was set
            uint256 newVersion = uint256(vm.load(address(diamond), versionSlot));
            console.log("Storage version after setting:", newVersion);
        } else {
            console.log("Storage version already set to:", currentVersion);
        }
        
        // Now initialize facets (they should work because storage version is set)
        ...
        console.log("Testing ceremony functions...");
        try CeremonyFacet(address(diamond)).startCeremony(1) {
            console.log("startCeremony successful!");
        } catch Error(string memory reason) {
            console.log("startCeremony failed:", reason);
        }
        ...
        console.log("Storage version properly initialized!");
    }
}